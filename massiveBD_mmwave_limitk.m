...
for isnr = 1 : nSNR
    ...
    for ichannel = 1 : channNum   
        ...
        rateLF(isnr) = rateLF(isnr) + calRateMU(H, Tlf, Wlf, Rs, Ns*ones(K,1));  
...
%         rateRxMmse(isnr) = rateRxMmse(isnr) + calRateMU(H, Tmmse, WRxMmse, Rs, Ns*ones(K,1));      
        ...
        rateAL(isnr) = rateAL(isnr) + calRateMU(Aal, Tal, Wal, Rs, Ns*ones(K, 1));
...
rateMat = [ratephaseBD_dft, rateLF, rateAL];
...
plot(SNR, abs(ratephaseBD_dft), 'b--', 'LineWidth', lw, 'MarkerSize', ms)
plot(SNR, abs(rateAL), 'k-v', 'LineWidth', lw, 'MarkerSize', ms)
plot(SNR, abs(rateLF), 'g-o', 'LineWidth', lw, 'MarkerSize', ms)

hold off
legend('Hybrid BD', 'Upper Bound', 'Limited Feedback')
xlabel('SNR (dB)')
