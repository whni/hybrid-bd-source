...
K = 8;      % User number
Ns = 16;     % #streams per user
Lr = 16;     % #chains per user
Lt = Lr*K;  % #chain at BS

% randn('state', 3);
Ncls = 8; 
Nray = 10;
channNum = 5;  % For each user
...
%         TphaseBD = Trf*TBD*Fwf;
%         
%         ratephaseBD_ar(isnr) = ratephaseBD_ar(isnr) + calRateMU(H, TphaseBD, WphaseBD, Rs, Ns*ones(K,1));  
%         
...
%         TphaseBD = Trf*TBD*Fwf;
%         
%         ratephaseBD_svd_ar(isnr) = ratephaseBD_svd_ar(isnr) + calRateMU(H, TphaseBD, WphaseBD, Rs, Ns*ones(K,1));  
% 
...
rateMat = [rateBD, rateSpa, ratephaseBD_dft, ratephaseBD_svd_dft];
...
% plot(SNR, abs(ratephaseBD_ar), 'ro', 'LineWidth', lw, 'MarkerSize', ms)
plot(SNR, abs(ratephaseBD_svd_dft), 'ro', 'LineWidth', lw, 'MarkerSize', ms)
% plot(SNR, abs(ratephaseBD_svd_ar), 'bs', 'LineWidth', lw, 'MarkerSize', ms)
hold off
legend('BD', 'Spatially Sparse Hybrid', 'MMSE at RX', 'phase BD (DFT)',  'phase BD (SVD+DFT)')
xlabel('SNR (dB)')
