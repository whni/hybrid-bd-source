...
plot(SNR, abs(rateSpa), 'b-.', 'LineWidth', lw, 'MarkerSize', ms)
plot(SNR, abs(rateLF), 'g-.', 'LineWidth', lw, 'MarkerSize', ms)


hold off
legend('Full-Complexity BD', 'Hybrid BD', 'Spatially Sparse Coding', 'Limited Feedback')
xlabel('SNR (dB)')
...
filename = sprintf('LF-Nt%d-K%d-Nr%d-Ns%d-Lr%d-Lt%d-Ncls%d-Nray%d', Nt, K, Nr, Ns, ...
     Lr,Lt,Ncls,Nray);
